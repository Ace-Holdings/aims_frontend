...
export default function AdminSales() {
    ...
    const handleSalesSubmit = async (e: any) => {
        ...
        try {
            ...
            console.log('managed to generate pdf');
    ...
    const generateInvoicePdf = (sale: any) => {
        ...
        doc.addImage(logoBase64, "PNG", 155, 10, 50, 40); // (x, y, width, height)

        // Address Box on the left side below the logo
        const address = `Ace Holdings Limited\nP.O. Box 1246\nBlantyre, Blantyre\nMalawi\n\nMobile: +265999257356\nwww.ace.co.mw`;

        doc.setFontSize(8);
        doc.setFont("helvetica", "normal");
        doc.text(address, 14, 40); // Starting point of the address

        // Draw a horizontal line below the logo and address box, slightly higher position
        doc.setLineWidth(0.5); // Line thickness
        doc.line(10, 65, 200, 65); // (startX, startY, endX, endY) with adjusted startY

        // Title
        doc.setFont("helvetica", "bold");
        doc.setFontSize(20);
        doc.text("Invoice", 14, 75); // Title moved up, starting position adjusted
